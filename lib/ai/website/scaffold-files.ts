/**
 * Fixed project scaffold + curated UI stack for website generation.
 *
 * Uses real @radix-ui/react-slot (Button), class-variance-authority (Button,
 * Badge), and clsx (cn utility). The iframe runtime provides inline
 * implementations of these as builtins alongside React/ReactDOM.
 *
 * Instead of the LLM generating every file from scratch (configs, entry,
 * buttons, inputs, cards), this module provides stable, pre-written files
 * that are injected into the output after generation. The LLM focuses on
 * section components + App.tsx — the creative work.
 *
 * Injection flow:
 *   LLM generates → critic runs → injectScaffoldFiles() merges stable
 *   files into the raw delimited output → final content returned.
 *
 * Runtime constraint: the preview sandbox has React 18 UMD + Babel
 * Standalone + Tailwind CDN + inline implementations of @radix-ui/react-slot,
 * class-variance-authority, and clsx registered as builtins.
 */
...
// ─────────────────────────────────────────────────────
// UI PRIMITIVES — Radix-backed + custom components
//
// Button: Radix Slot (asChild) + cva variants — real shadcn pattern
// Badge: cva variants — real shadcn pattern
// Card, Section, Container, Heading, Input, Textarea: custom Tailwind-based
// Colors are applied via className by section components.
// ─────────────────────────────────────────────────────

const BUTTON_TSX = `import React from "react";
...
const INPUT_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
const TEXTAREA_TSX = `import React from "react";
import { cn } from "@/lib/utils";
  ...
  return (
    <div>
      ...
      <textarea
        ...
        rows={4}
        className={cn(
          "w-full rounded-lg border px-4 py-2.5 text-base outline-none transition-colors resize-none",
          className
        )}
        {...props}
      />
...
const BADGE_TSX = `import React from "react";
import { cva, type VariantProps } from "class-variance-authority";
import { cn } from "@/lib/utils";
...
export default function Badge({ className, size, ...props }: BadgeProps) {
  return (
...
export default function Card({ className, children }: CardProps) {
  return (
    <div className={cn("surface-card rounded-[calc(var(--radius)+6px)] border bg-card text-card-foreground shadow-sm p-6", className)}>
      {children}
    </div>
  );
}`;

const SECTION_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
const CONTAINER_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
const HEADING_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
export default function Heading({
  ...
  return (
    <div>
      {eyebrow && (
        <span className="block text-xs uppercase tracking-widest font-medium mb-3 opacity-60">
          {eyebrow}
        </span>
      )}
      <Tag className={cn(sizeStyles[size], className)}>{children}</Tag>
    </div>
  );
}`;

const SEPARATOR_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
const ACCORDION_TSX = `import React from "react";
...
const TABS_TSX = `import React from "react";
...
const DIALOG_TSX = `import React from "react";
  ...
  ({ className, children, ...props }, ref) => (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          "fixed left-[50%] top-[50%] z-50 w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-xl border bg-white p-6 shadow-xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95",
          className
        )}
        {...props}
      >
...
export { Dialog, DialogTrigger, DialogContent, DialogClose, DialogOverlay, DialogTitle, DialogDescription };`;

const SHEET_TSX = `import React from "react";
import * as DialogPrimitive from "@radix-ui/react-dialog";
import { cn } from "@/lib/utils";
...
const ASPECT_RATIO_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
export default function AspectRatio({ ratio = 16 / 9, className, children }: AspectRatioProps) {
  return (
    <div className={cn("relative w-full overflow-hidden", className)} style={{ paddingBottom: (1 / ratio) * 100 + "%" }}>
      <div className="absolute inset-0">{children}</div>
    </div>
  );
}`;

const TESTIMONIAL_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
export default function Testimonial({ quote, author, role, avatar, className }: TestimonialProps) {
  return (
    <figure className={cn("space-y-4", className)}>
      ...
      <figcaption className="flex items-center gap-3">
        ...
        <div>
          ...
          {role && <p className="text-xs opacity-60">{role}</p>}
        </div>
      </figcaption>
    </figure>
  );
...
const STATS_BAND_TSX = `import React from "react";
    ...
    <Section className={cn("py-12", className)}>
      <Container>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {stats.map((s) => (
            <div key={s.label}>
              <p className="text-3xl md:text-5xl font-bold">{s.value}</p>
              <p className="text-sm mt-1 opacity-60">{s.label}</p>
            </div>
          ))}
        </div>
      </Container>
...
const GALLERY_TSX = `import React from "react";
import { cn } from "@/lib/utils";
...
export default function Gallery({ images, className }: GalleryProps) {
  return (
    <div className={cn("grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-4", className)}>
      {images.map((img, i) => (
        <div key={i} className={cn("overflow-hidden rounded-lg", spanClasses[img.span || "normal"])}>
          <img
            src={img.src}
            alt={img.alt}
...
const MOBILE_NAV_TSX = `import React from "react";
...
export default function MobileNav({ links, brandName, ctaLabel, ctaHref, className }: MobileNavProps) {
  return (
    <Sheet>
      <SheetTrigger asChild>
        <button className={cn("md:hidden p-2", className)} aria-label="Open menu">
      ...
      <SheetContent side="right" className="flex flex-col pt-16">
        <p className="text-lg font-semibold mb-8">{brandName}</p>
        <nav className="flex flex-col gap-4">
          {links.map((link) => (
            <SheetClose key={link.href} asChild>
              <a href={link.href} className="text-lg py-2 border-b border-zinc-100 hover:text-zinc-600 transition-colors">
                {link.label}
              </a>
            </SheetClose>
          ))}
...
// ─────────────────────────────────────────────────────
// CONFIG FILES — metadata only (not used at runtime)
// ─────────────────────────────────────────────────────
...
const ESLINT_CONFIG_JS = `import js from "@eslint/js";
...
const README_MD = `# Cenate Website

This project was generated by Cenate with a fixed Lovable-style Vite + React + TypeScript scaffold.
...
const BUN_LOCK = `{
  ...
  "packages": {
    "@adobe/css-tools": ["@adobe/css-tools@4.4.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@adobe/css-tools/-/css-tools-4.4.4.tgz", {}, "sha512-Elp+iwUx5rN5+Y8xLt5/GRoG20WGoDCQ/1Fb+1LiGtvwbDavuSk0jhD/eZdckHAuzcDzccnkv+rEjyWfRx18gg=="],

    "@alloc/quick-lru": ["@alloc/quick-lru@5.2.0", "", {}, "sha512-UrcABB+4bUrFABwbluTIBErXwvbsU/V7TZWfmbgJfbkwiBuziS9gxdODUyuiecfdGQ85jglMW6juS3+z5TsKLw=="],

    "@babel/code-frame": ["@babel/code-frame@7.29.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@babel/code-frame/-/code-frame-7.29.0.tgz", { "dependencies": { "@babel/helper-validator-identifier": "^7.28.5", "js-tokens": "^4.0.0", "picocolors": "^1.1.1" } }, "sha512-9NhCeYjq9+3uxgdtp20LSiJXJvN0FeCtNGpJxuMFZ1Kv3cWUNb6DOhJwUvcVCzKGR66cw4njwM6hrJLqgOwbcw=="],
    ...
    "@jridgewell/sourcemap-codec": ["@jridgewell/sourcemap-codec@1.5.5", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@jridgewell/sourcemap-codec/-/sourcemap-codec-1.5.5.tgz", {}, "sha512-cYQ9310grqxueWbl+WuIUIaiUaDcj7WOq5fVhEljNVgRfOUhY9fy2zTvfoqWsnebh8Sl70VScFbICvJnLKB0Og=="],
    ...
    "@testing-library/dom": ["@testing-library/dom@10.4.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@testing-library/dom/-/dom-10.4.1.tgz", { "dependencies": { "@babel/code-frame": "^7.10.4", "@babel/runtime": "^7.12.5", "@types/aria-query": "^5.0.1", "aria-query": "5.3.0", "dom-accessibility-api": "^0.5.9", "lz-string": "^1.5.0", "picocolors": "1.1.1", "pretty-format": "^27.0.2" } }, "sha512-o4PXJQidqJl82ckFaXUeoAW+XysPLauYI43Abki5hABd853iMhitooc6znOnczgbTYmEP6U6/y1ZyKAIsvMKGg=="],

    "@testing-library/jest-dom": ["@testing-library/jest-dom@6.9.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@testing-library/jest-dom/-/jest-dom-6.9.1.tgz", { "dependencies": { "@adobe/css-tools": "^4.4.0", "aria-query": "^5.0.0", "css.escape": "^1.5.1", "dom-accessibility-api": "^0.6.3", "picocolors": "^1.1.1", "redent": "^3.0.0" } }, "sha512-zIcONa+hVtVSSep9UT3jZ5rizo2BsxgyDYU7WFD5eICBE7no3881HGeb/QkGfsJs6JTkY1aQhT7rIPC7e+0nnA=="],

    "@testing-library/react": ["@testing-library/react@16.3.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@testing-library/react/-/react-16.3.2.tgz", { "dependencies": { "@babel/runtime": "^7.12.5" }, "peerDependencies": { "@testing-library/dom": "^10.0.0", "@types/react": "^18.0.0 || ^19.0.0", "@types/react-dom": "^18.0.0 || ^19.0.0", "react": "^18.0.0 || ^19.0.0", "react-dom": "^18.0.0 || ^19.0.0" }, "optionalPeers": ["@types/react", "@types/react-dom"] }, "sha512-XU5/SytQM+ykqMnAnvB2umaJNIOsLF3PVv//1Ew4CTcpz0/BRyy/af40qqrt7SjKpDdT1saBMc42CUok5gaw+g=="],

    "@tootallnate/once": ["@tootallnate/once@2.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@tootallnate/once/-/once-2.0.0.tgz", {}, "sha512-XCuKFP5PS55gnMVu3dty8KPatLqUoy/ZYzDzAGCQ8JNFCkLXzmI7vNHCR+XpbZaMWQK/vQubr7PkYq8g470J/A=="],

    "@types/aria-query": ["@types/aria-query@5.0.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@types/aria-query/-/aria-query-5.0.4.tgz", {}, "sha512-rfT93uj5s0PRL7EzccGMs3brplhcrghnDoV26NqKhCAS1hVo+WdNsPvE/yb6ilfr5hi2MEk6d5EWJTKdxg8jVw=="],

    "@types/chai": ["@types/chai@5.2.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@types/chai/-/chai-5.2.3.tgz", { "dependencies": { "@types/deep-eql": "*", "assertion-error": "^2.0.1" } }, "sha512-Mw558oeA9fFbv65/y4mHtXDs9bPnFMZAL/jxdPFUpOHHIXX91mcgEHbS5Lahr+pwZFR8A7GQleRWeI6cGFC2UA=="],
    ...
    "@types/deep-eql": ["@types/deep-eql@4.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@types/deep-eql/-/deep-eql-4.0.2.tgz", {}, "sha512-c9h9dVVMigMPc4bwTvC5dxqtqJZwQPePsWjPlpSOnojbor6pGqdk541lfA7AqFQr5pB1BRdq0juY9db81BwyFw=="],
    ...
    "@vitest/mocker": ["@vitest/mocker@3.2.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@vitest/mocker/-/mocker-3.2.4.tgz", { "dependencies": { "@vitest/spy": "3.2.4", "estree-walker": "^3.0.3", "magic-string": "^0.30.17" }, "peerDependencies": { "msw": "^2.4.9", "vite": "^5.0.0 || ^6.0.0 || ^7.0.0-0" }, "optionalPeers": ["msw", "vite"] }, "sha512-46ryTE9RZO/rfDd7pEqFl7etuyzekzEhUbTW3BvmeO/BcCMEgq59BKhek3dXDWgAj4oMK6OZi+vRr1wPW6qjEQ=="],

    "@vitest/pretty-format": ["@vitest/pretty-format@3.2.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@vitest/pretty-format/-/pretty-format-3.2.4.tgz", { "dependencies": { "tinyrainbow": "^2.0.0" } }, "sha512-IVNZik8IVRJRTr9fxlitMKeJeXFFFN0JaB9PHPGQ8NKQbGpfjlTx9zO4RefN8gp7eqjNy8nyK3NZmBzOPeIxtA=="],

    "@vitest/runner": ["@vitest/runner@3.2.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@vitest/runner/-/runner-3.2.4.tgz", { "dependencies": { "@vitest/utils": "3.2.4", "pathe": "^2.0.3", "strip-literal": "^3.0.0" } }, "sha512-oukfKT9Mk41LreEW09vt45f8wx7DordoWUZMYdY/cyAk7w5TWkTRCNZYF7sX7n2wB7jyGAl74OxgwhPgKaqDMQ=="],
    ...
    "@vitest/spy": ["@vitest/spy@3.2.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@vitest/spy/-/spy-3.2.4.tgz", { "dependencies": { "tinyspy": "^4.0.3" } }, "sha512-vAfasCOe6AIK70iP5UD11Ac4siNUNJ9i/9PZ3NKx07sG6sUxeag1LWdNrMWeKKYBLlzuK+Gn65Yd5nyL6ds+nw=="],

    "@vitest/utils": ["@vitest/utils@3.2.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/@vitest/utils/-/utils-3.2.4.tgz", { "dependencies": { "@vitest/pretty-format": "3.2.4", "loupe": "^3.1.4", "tinyrainbow": "^2.0.0" } }, "sha512-fB2V0JFrQSMsCo9HiSq3Ezpdv4iYaXRG1Sx8edX3MwxfyNn83mKiGzOcH+Fkxt4MHxr3y42fQi1oeAInqgX2QA=="],

    "abab": ["abab@2.0.6", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/abab/-/abab-2.0.6.tgz", {}, "sha512-j2afSsaIENvHZN2B8GOpF566vZ5WVk5opAiMTvWgaQT8DkbOqsTfvNAvHoRGU2zzP8cPoqys+xHTRDWW8L+/BA=="],

    "acorn": ["acorn@8.15.0", "", { "bin": "bin/acorn" }, "sha512-NZyJarBfL7nWwIq+FDL6Zp/yHEhePMNnnJ0y3qfieCrmNvYct8uvtiV41UvlSe6apAfk0fY1FbWx+NwfmpvtTg=="],

    "acorn-globals": ["acorn-globals@7.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/acorn-globals/-/acorn-globals-7.0.1.tgz", { "dependencies": { "acorn": "^8.1.0", "acorn-walk": "^8.0.2" } }, "sha512-umOSDSDrfHbTNPuNpC2NSnnA3LUrqpevPb4T9jRx4MagXNS0rs+gwiTcAvqCRmsD6utzsrzNt+ebm00SNWiC3Q=="],

    "acorn-jsx": ["acorn-jsx@5.3.2", "", { "peerDependencies": { "acorn": "^6.0.0 || ^7.0.0 || ^8.0.0" } }, "sha512-rq9s+JNhf0IChjtDXxllJ7g41oZk5SlXtp0LHwyA5cejwn7vKmKp4pPri6YEePv2PU65sAsegbXtIinmDFDXgQ=="],

    "acorn-walk": ["acorn-walk@8.3.5", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/acorn-walk/-/acorn-walk-8.3.5.tgz", { "dependencies": { "acorn": "^8.11.0" } }, "sha512-HEHNfbars9v4pgpW6SO1KSPkfoS0xVOM/9UzkJltjlsHZmJasxg8aXkuZa7SMf8vKGIBhpUsPluQSqhJFCqebw=="],

    "agent-base": ["agent-base@6.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/agent-base/-/agent-base-6.0.2.tgz", { "dependencies": { "debug": "4" } }, "sha512-RZNwNclF7+MS/8bDg70amg32dyeZGZxiDuQmZxKLAlQjr3jGyLx+4Kkk58UO7D2QdgFIQCovuSuZESne6RG6XQ=="],
    ...
    "aria-query": ["aria-query@5.3.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/aria-query/-/aria-query-5.3.2.tgz", {}, "sha512-COROpnaoap1E2F000S62r6A60uHZnmlvomhfyT2DlTcrY1OrBKn2UhH7qn5wTC9zMvD0AY7csdPSNwKP+7WiQw=="],

    "assertion-error": ["assertion-error@2.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/assertion-error/-/assertion-error-2.0.1.tgz", {}, "sha512-Izi8RQcffqCeNVgFigKli1ssklIbpHnCYc6AknXGYoB6grJqyeby7jv12JUQgmTAnIDnbck1uxksT4dzN3PWBA=="],

    "asynckit": ["asynckit@0.4.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/asynckit/-/asynckit-0.4.0.tgz", {}, "sha512-Oei9OH4tRh0YqU3GxhX79dM/mwVgvbZJaSNaRk+bshkj0S5cfHcgYakreBjrHwatXKbz+IoIdYLxrKim2MjW0Q=="],
    ...
    "cac": ["cac@6.7.14", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/cac/-/cac-6.7.14.tgz", {}, "sha512-b6Ilus+c3RrdDk+JhLKUAQfzzgLEPy6wcXqS7f/xe1EETvsDP6GORG7SFuOs6cID5YkqchW/LXZbX5bc8j7ZcQ=="],

    "call-bind-apply-helpers": ["call-bind-apply-helpers@1.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/call-bind-apply-helpers/-/call-bind-apply-helpers-1.0.2.tgz", { "dependencies": { "es-errors": "^1.3.0", "function-bind": "^1.1.2" } }, "sha512-Sp1ablJ0ivDkSzjcaJdxEunN5/XvksFJ2sMBFfq6x0ryhQV/2b/KwFe21cMpmHtPOSij8K99/wSfoEuTObmuMQ=="],
    ...
    "chai": ["chai@5.3.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/chai/-/chai-5.3.3.tgz", { "dependencies": { "assertion-error": "^2.0.1", "check-error": "^2.1.1", "deep-eql": "^5.0.1", "loupe": "^3.1.0", "pathval": "^2.0.0" } }, "sha512-4zNhdJD/iOjSH0A05ea+Ke6MU5mmpQcbQsSOkgdaUMJ9zTlDTD/GYlwohmIE2u0gaxHYiVHEn1Fw9mZ/ktJWgw=="],

    "chalk": ["chalk@4.1.2", "", { "dependencies": { "ansi-styles": "^4.1.0", "supports-color": "^7.1.0" } }, "sha512-oKnbhFyRIXpUuez8iBMmyEa4nbj4IOQyuhc/wy9kY7/WVPcwIO9VA668Pu8RkO7+0G76SLROeyw9CpQ061i4mA=="],

    "check-error": ["check-error@2.1.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/check-error/-/check-error-2.1.3.tgz", {}, "sha512-PAJdDJusoxnwm1VwW07VWwUN1sl7smmC3OKggvndJFadxxDRyFJBX/ggnu/KE4kQAB7a3Dp8f/YXC1FlUprWmA=="],
    ...
    "combined-stream": ["combined-stream@1.0.8", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/combined-stream/-/combined-stream-1.0.8.tgz", { "dependencies": { "delayed-stream": "~1.0.0" } }, "sha512-FQN4MRfuJeHf7cBbBMJFXhKSDq+2kAArBlmRBvcvFE5BB1HZKXtSFASDhdlz9zOYwxh8lDdnvmMOe/+5cdoEdg=="],
    ...
    "css.escape": ["css.escape@1.5.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/css.escape/-/css.escape-1.5.1.tgz", {}, "sha512-YUifsXXuknHlUsmlgyY0PKzgPOr7/FjCePfHNt0jxm83wHZi44VDMQ7/fGNkjY3/jV1MC+1CmZbaHzugyeRtpg=="],
    ...
    "cssom": ["cssom@0.5.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/cssom/-/cssom-0.5.0.tgz", {}, "sha512-iKuQcq+NdHqlAcwUY0o/HL69XQrUaQdMjmStJ8JFmUaiiQErlhrmuigkg/CU4E2J0IyUKUrMAgl36TvN67MqTw=="],

    "cssstyle": ["cssstyle@2.3.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/cssstyle/-/cssstyle-2.3.0.tgz", { "dependencies": { "cssom": "~0.3.6" } }, "sha512-AZL67abkUzIuvcHqk7c09cezpGNcxUxU4Ioi/05xHk4DQeTkWmGYftIE6ctU6AEt+Gn4n1lDStOtj7FKycP71A=="],
    ...
    "data-urls": ["data-urls@3.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/data-urls/-/data-urls-3.0.2.tgz", { "dependencies": { "abab": "^2.0.6", "whatwg-mimetype": "^3.0.0", "whatwg-url": "^11.0.0" } }, "sha512-Jy/tj3ldjZJo63sVAvg6LHt2mHvl4V6AgRAmNDtLdm7faqtsx+aJG42rsyCo9JCoRVKwPFzKlIPx3DIibwSIaQ=="],
    ...
    "decimal.js": ["decimal.js@10.6.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/decimal.js/-/decimal.js-10.6.0.tgz", {}, "sha512-YpgQiITW3JXGntzdUmyUR1V812Hn8T1YVXhCu+wO3OpS4eU9l4YdD3qjyiKdV6mvV29zapkMeD390UVEf2lkUg=="],

    "decimal.js-light": ["decimal.js-light@2.5.1", "", {}, "sha512-qIMFpTMZmny+MMIitAB6D7iVPEorVw6YQRWkvarTkT4tBeSLLiHzcwj6q0MmYSFCiVpiqPJTJEYIrpcPzVEIvg=="],

    "deep-eql": ["deep-eql@5.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/deep-eql/-/deep-eql-5.0.2.tgz", {}, "sha512-h5k/5U50IJJFpzfL6nO9jaaumfjO/f2NjK/oYB2Djzm4p9L+3T9qWpZqZ2hAbLPuuYq9wrU08WQyBTL5GbPk5Q=="],

    "deep-is": ["deep-is@0.1.4", "", {}, "sha512-oIPzksmTg4/MriiaYGO+okXDT7ztn/w3Eptv/+gSIdMdKsJo0u4CfYNFJPy+4SKMuCqGw2wxnA+URMg3t8a/bQ=="],

    "delayed-stream": ["delayed-stream@1.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/delayed-stream/-/delayed-stream-1.0.0.tgz", {}, "sha512-ZySD7Nf91aLB0RxL4KGrKHBXl7Eds1DAmEdcoVawXnLD7SDhpNgtuII2aAkg7a7QS41jxPSZ17p4VdGnMHk3MQ=="],

    "dequal": ["dequal@2.0.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/dequal/-/dequal-2.0.3.tgz", {}, "sha512-0je+qPKHEMohvfRTCEo3CrPG6cAzAYgmzKyxRiYSSDkS6eGJdyVJm7WaYA5ECaAD9wLB2T4EEeymA5aFVcYXCA=="],
    ...
    "dom-accessibility-api": ["dom-accessibility-api@0.6.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/dom-accessibility-api/-/dom-accessibility-api-0.6.3.tgz", {}, "sha512-7ZgogeTnjuHbo+ct10G9Ffp0mif17idi0IyWNVA/wcwcm7NPOD/WEHVP3n7n3MhXqxoIYm8d6MuZohYWIZ4T3w=="],

    "dom-helpers": ["dom-helpers@5.2.1", "", { "dependencies": { "@babel/runtime": "^7.8.7", "csstype": "^3.0.2" } }, "sha512-nRCa7CK3VTrM2NmGkIy4cbK7IZlgBE/PYMn55rrXefr5xXDP0LdtfPnblFDoVdcAfslJ7or6iqAUnx0CCGIWQA=="],

    "domexception": ["domexception@4.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/domexception/-/domexception-4.0.0.tgz", { "dependencies": { "webidl-conversions": "^7.0.0" } }, "sha512-A2is4PLG+eeSfoTMA95/s4pvAoSo2mKtiM5jlHkAVewmiO8ISFTFKZjH7UAM1Atli/OT/7JHOrJRJiMKUZKYBw=="],

    "dunder-proto": ["dunder-proto@1.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/dunder-proto/-/dunder-proto-1.0.1.tgz", { "dependencies": { "call-bind-apply-helpers": "^1.0.1", "es-errors": "^1.3.0", "gopd": "^1.2.0" } }, "sha512-KIN/nDJBQRcXw0MLVhZE9iQHmG68qAVIBg9CqmUYjmQIhgij9U5MFvrqkUL5FbtyyzZuOeOt0zdeRe4UY7ct+A=="],
    ...
    "entities": ["entities@6.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/entities/-/entities-6.0.1.tgz", {}, "sha512-aN97NXWF6AWBTahfVOIrB/NShkzi5H7F9r1s9mD3cDj4Ko5f2qhhVoYMibXF7GlLveb/D2ioWay8lxI97Ven3g=="],

    "es-define-property": ["es-define-property@1.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/es-define-property/-/es-define-property-1.0.1.tgz", {}, "sha512-e3nRfgfUZ4rNGL232gUgX06QNyyez04KdjFrF+LTRoOXmrOgFKDg4BCdsjW8EnT69eqdYGmRpJwiPVYNrCaW3g=="],

    "es-errors": ["es-errors@1.3.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/es-errors/-/es-errors-1.3.0.tgz", {}, "sha512-Zf5H2Kxt2xjTvbJvP2ZWLEICxA6j+hAmMzIlypy4xcBg1vKVnx89Wy0GbS+kf5cwCVFFzdCFh2XSCFNULS6csw=="],

    "es-module-lexer": ["es-module-lexer@1.7.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/es-module-lexer/-/es-module-lexer-1.7.0.tgz", {}, "sha512-jEQoCwk8hyb2AZziIOLhDqpm5+2ww5uIE6lkO/6jcOCusfk6LhMHpXXfBLXTZ7Ydyt0j4VoUQv6uGNYbdW+kBA=="],

    "es-object-atoms": ["es-object-atoms@1.1.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/es-object-atoms/-/es-object-atoms-1.1.1.tgz", { "dependencies": { "es-errors": "^1.3.0" } }, "sha512-FGgH2h8zKNim9ljj7dankFPcICIK9Cp5bm+c2gQSYePhpaG5+esrLODihIorn+Pe6FGJzWhXQotPv73jTaldXA=="],

    "es-set-tostringtag": ["es-set-tostringtag@2.1.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/es-set-tostringtag/-/es-set-tostringtag-2.1.0.tgz", { "dependencies": { "es-errors": "^1.3.0", "get-intrinsic": "^1.2.6", "has-tostringtag": "^1.0.2", "hasown": "^2.0.2" } }, "sha512-j6vWzfrGVfyXxge+O0x5sh6cvxAog0a/4Rdd2K36zCMV5eJ+/+tOAngRO8cODMNWbVRdVlmGZQL2YS3yR8bIUA=="],
    ...
    "esprima": ["esprima@4.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/esprima/-/esprima-4.0.1.tgz", { "bin": { "esparse": "./bin/esparse.js", "esvalidate": "./bin/esvalidate.js" } }, "sha512-eGuFFw7Upda+g4p+QHvnW0RyTX/SVeJBDM/gCtMARO0cLuT2HcEKnTPvhjV6aGeqrCB/sbNop0Kszm0jsaWU4A=="],
    ...
    "estree-walker": ["estree-walker@3.0.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/estree-walker/-/estree-walker-3.0.3.tgz", { "dependencies": { "@types/estree": "^1.0.0" } }, "sha512-7RUKfXgSMMkzt6ZuXmqapOurLGPPfgj6l9uRZ7lRGolvk0y2yocc35LdcxKC5PQZdn2DMqioAQ2NoWcrTKmm6g=="],

    "esutils": ["esutils@2.0.3", "", {}, "sha512-kVscqXk4OCp68SZ0dkgEKVi6/8ij300KBWTJq32P/dYeWTSwK41WyTxalN1eRmA5Z9UU/LX9D7FWSmV9SAYx6g=="],

    "eventemitter3": ["eventemitter3@4.0.7", "", {}, "sha512-8guHBZCwKnFhYdHr2ysuRWErTwhoN2X8XELRlrRwpmfeY2jjuUN4taQMsULKUVo1K4DvZl+0pgfyoysHxvmvEw=="],

    "expect-type": ["expect-type@1.3.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/expect-type/-/expect-type-1.3.0.tgz", {}, "sha512-knvyeauYhqjOYvQ66MznSMs83wmHrCycNEN6Ao+2AeYEfxUIkuiVxdEa1qlGEPK+We3n0THiDciYSsCcgW/DoA=="],
    ...
    "fdir": ["fdir@6.5.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/fdir/-/fdir-6.5.0.tgz", { "peerDependencies": { "picomatch": "^3 || ^4" }, "optionalPeers": ["picomatch"] }, "sha512-tIbYtZbucOs0BRGqPJkshJUYdL+SDH7dVM8gjy+ERp3WAUjLEFJE+02kanyHtwjWOnwrKYBiwAmM0p4kLJAnXg=="],
    ...
    "get-proto": ["get-proto@1.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/get-proto/-/get-proto-1.0.1.tgz", { "dependencies": { "dunder-proto": "^1.0.1", "es-object-atoms": "^1.0.0" } }, "sha512-sTSfBjoXBp89JvIKIefqw7U2CCebsc74kiY6awiGogKtoSGbgjYE/G/+l9sF3MWFPNc9IcoOC4ODfKHfxFmp0g=="],
    ...
    "gopd": ["gopd@1.2.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/gopd/-/gopd-1.2.0.tgz", {}, "sha512-ZUKRh6/kUFoAiTAtTYPZJ3hw9wNxx+BIBOijnlG9PnrJsCcSjs1wyyD6vJpaYtgnzDrKYRSqf3OO6Rfa93xsRg=="],

    "graphemer": ["graphemer@1.4.0", "", {}, "sha512-EtKwoO6kxCL9WO5xipiHTZlSzBm7WLT627TqC/uVRd0HKmq8NXyebnNYxDoBi7wt8eTWrUrKXCOVaFq9x1kgag=="],

    "has-flag": ["has-flag@4.0.0", "", {}, "sha512-EykJT/Q1KjTWctppgIAgfSO0tKVuZUjhgMr17kqTumMl6Afv3EISleU7qZUzoXDFTAHTDC4NOoG/ZxU3EvlMPQ=="],

    "has-symbols": ["has-symbols@1.1.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/has-symbols/-/has-symbols-1.1.0.tgz", {}, "sha512-1cDNdwJ2Jaohmb3sg4OmKaMBwuC48sYni5HUw2DvsC8LjGTLK9h+eb1X6RyuOHe4hT0ULCW68iomhjUoKUqlPQ=="],

    "has-tostringtag": ["has-tostringtag@1.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/has-tostringtag/-/has-tostringtag-1.0.2.tgz", { "dependencies": { "has-symbols": "^1.0.3" } }, "sha512-NqADB8VjPFLM2V0VvHUewwwsw0ZWBaIdgo+ieHtK3hasLz4qeCRjYcqfB6AQrBggRKppKF8L52/VqdVsO47Dlw=="],

    "hasown": ["hasown@2.0.2", "", { "dependencies": { "function-bind": "^1.1.2" } }, "sha512-0hJU9SCPvmMzIBdZFqNPXWa6dqh7WdH0cII9y+CyS8rG3nL48Bclra9HmKhVVUHyPWNH5Y7xDwAB7bfgSjkUMQ=="],

    "html-encoding-sniffer": ["html-encoding-sniffer@3.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/html-encoding-sniffer/-/html-encoding-sniffer-3.0.0.tgz", { "dependencies": { "whatwg-encoding": "^2.0.0" } }, "sha512-oWv4T4yJ52iKrufjnyZPkrN0CH3QnrUqdB6In1g5Fe1mia8GmF36gnfNySxoZtxD5+NmYw1EElVXiBk93UeskA=="],

    "http-proxy-agent": ["http-proxy-agent@5.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/http-proxy-agent/-/http-proxy-agent-5.0.0.tgz", { "dependencies": { "@tootallnate/once": "2", "agent-base": "6", "debug": "4" } }, "sha512-n2hY8YdoRE1i7r6M0w9DIw5GgZN0G25P8zLCRQ8rjXtTU3vsNFBI/vWK/UIeE6g5MUUz6avwAPXmL6Fy9D/90w=="],

    "https-proxy-agent": ["https-proxy-agent@5.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/https-proxy-agent/-/https-proxy-agent-5.0.1.tgz", { "dependencies": { "agent-base": "6", "debug": "4" } }, "sha512-dFcAjpTQFgoLMzC2VwU+C/CbS7uRL0lWmxDITmqm7C+7F0Odmj6s9l6alZc6AELXhrnggM2CeWSXHGOdX2YtwA=="],

    "iconv-lite": ["iconv-lite@0.6.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/iconv-lite/-/iconv-lite-0.6.3.tgz", { "dependencies": { "safer-buffer": ">= 2.1.2 < 3.0.0" } }, "sha512-4fCk79wshMdzMp2rH06qWrJE4iolqLhCUH+OiuIgU++RB0+94NlDL81atO7GX55uUKueo0txHNtvEyI6D7WdMw=="],
    ...
    "indent-string": ["indent-string@4.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/indent-string/-/indent-string-4.0.0.tgz", {}, "sha512-EdDDZu4A2OyIK7Lr/2zG+w5jmbuk1DVBnEwREQvBzspBJkCEbRa8GxU1lghYcaGJCnRWibjDXlq779X1/y5xwg=="],
    ...
    "loupe": ["loupe@3.2.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/loupe/-/loupe-3.2.1.tgz", {}, "sha512-CdzqowRJCeLU72bHvWqwRBBlLcMEtIvGrlvef74kMnV2AolS9Y8xUv1I0U/MNAWMhBlKIoyuEgoJ0t/bbwHbLQ=="],
    ...
    "lz-string": ["lz-string@1.5.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/lz-string/-/lz-string-1.5.0.tgz", { "bin": { "lz-string": "bin/bin.js" } }, "sha512-h5bgJWpxJNswbU7qCrV0tIKQCaS3blPDrqKWx+QxzuzL1zGUzij9XCWLrSLsJPu5t+eWA/ycetzYAO5IOMcWAQ=="],

    "magic-string": ["magic-string@0.30.21", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/magic-string/-/magic-string-0.30.21.tgz", { "dependencies": { "@jridgewell/sourcemap-codec": "^1.5.5" } }, "sha512-vd2F4YUyEXKGcLHoq+TEyCjxueSeHnFxyyjNp80yg0XV4vUhnDer/lvvlqM/arB5bXQN5K2/3oinyCRyx8T2CQ=="],

    "math-intrinsics": ["math-intrinsics@1.1.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/math-intrinsics/-/math-intrinsics-1.1.0.tgz", {}, "sha512-/IXtbwEk5HTPyEwyKX6hGkYXxM9nbj64B+ilVJnC/R6B0pH5G4V3b0pVbL7DBj4tkhBAppbQUlf6F6Xl9LHu1g=="],

    "merge2": ["merge2@1.4.1", "", {}, "sha512-8q7VEgMJW4J8tcfVPy8g09NcQwZdbwFEqhe/WZkoIzjn/3TGDwtOCYtXGxA3O8tPzpczCCDgv+P2P5y00ZJOOg=="],

    "micromatch": ["micromatch@4.0.8", "", { "dependencies": { "braces": "^3.0.3", "picomatch": "^2.3.1" } }, "sha512-PXwfBhYu0hBCPw8Dn0E+WDYb7af3dSLVWKi3HGv84IdF4TyFoC0ysxFd0Goxw7nSv4T/PzEJQxsYsEiFCKo2BA=="],

    "mime-db": ["mime-db@1.52.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/mime-db/-/mime-db-1.52.0.tgz", {}, "sha512-sPU4uV7dYlvtWJxwwxHD0PuihVNiE7TyAbQ5SWxDCB9mUYvOgroQOwYQQOKPJ8CIbE+1ETVlOoK1UC2nU3gYvg=="],

    "mime-types": ["mime-types@2.1.35", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/mime-types/-/mime-types-2.1.35.tgz", { "dependencies": { "mime-db": "1.52.0" } }, "sha512-ZDY+bPm5zTTF+YpCrAU9nK0UgICYPT0QtT1NZWFv4s++TNkcgVaT0g6+4R2uI4MjQjzysHB1zxuWL50hzaeXiw=="],

    "min-indent": ["min-indent@1.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/min-indent/-/min-indent-1.0.1.tgz", {}, "sha512-I9jwMn07Sy/IwOj3zVkVik2JTvgpaykDZEigL6Rx6N9LbMywwUSMtxET+7lVoDLLd3O3IXwJwvuuns8UB/HeAg=="],
    ...
    "nwsapi": ["nwsapi@2.2.23", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/nwsapi/-/nwsapi-2.2.23.tgz", {}, "sha512-7wfH4sLbt4M0gCDzGE6vzQBo0bfTKjU7Sfpqy/7gs1qBfYz2vEJH6vXcBKpO3+6Yu1telwd0t9HpyOoLEQQbIQ=="],
    ...
    "parse5": ["parse5@7.3.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/parse5/-/parse5-7.3.0.tgz", { "dependencies": { "entities": "^6.0.0" } }, "sha512-IInvU7fabl34qmi9gY8XOVxhYyMyuH2xUNpb2q8/Y+7552KlejkRvqvD19nMoUW/uQGGbqNpA6Tufu5FL5BZgw=="],
    ...
    "pathe": ["pathe@2.0.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/pathe/-/pathe-2.0.3.tgz", {}, "sha512-WUjGcAqP1gQacoQe+OBJsFA7Ld4DyXuUIjZ5cc75cLHvJ7dtNsTugphxIADwspS+AraAUePCKrSVtPLFj/F88w=="],

    "pathval": ["pathval@2.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/pathval/-/pathval-2.0.1.tgz", {}, "sha512-//nshmD55c46FuFw26xV/xFAaB5HF9Xdap7HJBBnrKdAd6/GxDBaNA1870O79+9ueg61cZLSVc+OaFlfmObYVQ=="],

    "picocolors": ["picocolors@1.1.1", "", {}, "sha512-xceH2snhtb5M9liqDsmEw56le376mTZkEX/jEb/RxNFyegNul7eNslCXP9FDj/Lcu0X8KEyMceP2ntpaHrDEVA=="],

    "picomatch": ["picomatch@4.0.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/picomatch/-/picomatch-4.0.4.tgz", {}, "sha512-QP88BAKvMam/3NxH6vj2o21R6MjxZUAd6nlwAS/pnGvN9IVLocLHxGYIzFhg6fUQ+5th6P4dv4eW9jX3DSIj7A=="],
    ...
    "pretty-format": ["pretty-format@27.5.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/pretty-format/-/pretty-format-27.5.1.tgz", { "dependencies": { "ansi-regex": "^5.0.1", "ansi-styles": "^5.0.0", "react-is": "^17.0.1" } }, "sha512-Qb1gy5OrP5+zDf2Bvnzdl3jsTf1qXVMazbvCoKhtKqVs4/YK4ozX4gKQJJVyNe+cajNPn0KoC0MC3FUmaHWEmQ=="],

    "prop-types": ["prop-types@15.8.1", "", { "dependencies": { "loose-envify": "^1.4.0", "object-assign": "^4.1.1", "react-is": "^16.13.1" } }, "sha512-oj87CgZICdulUohogVAR7AjlC0327U4el4L6eAvOqCeudMDVU0NThNaV+b9Df4dXgSP1gXMTnPdhfe/2qDH5cg=="],

    "psl": ["psl@1.15.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/psl/-/psl-1.15.0.tgz", { "dependencies": { "punycode": "^2.3.1" } }, "sha512-JZd3gMVBAVQkSs6HdNZo9Sdo0LNcQeMNP3CozBJb3JYC/QUYZTnKxP+f8oWRX4rHP5EurWxqAHTSwUCjlNKa1w=="],

    "punycode": ["punycode@2.3.1", "", {}, "sha512-vYt7UD1U9Wg6138shLtLOvdAu+8DsC/ilFtEVHcH+wydcSpNE20AfSOduf6MkRFahL5FY7X1oU7nKVZFtfq8Fg=="],

    "querystringify": ["querystringify@2.2.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/querystringify/-/querystringify-2.2.0.tgz", {}, "sha512-FIqgj2EUvTa7R50u0rGsyTftzjYmv/a3hO345bZNrqabNqjtgiDMgmo4mkUjd+nzU5oF3dClKqFIPUKybUyqoQ=="],
    ...
    "redent": ["redent@3.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/redent/-/redent-3.0.0.tgz", { "dependencies": { "indent-string": "^4.0.0", "strip-indent": "^3.0.0" } }, "sha512-6tDA8g98We0zd0GvVeMT9arEOnTw9qM03L9cJXaCjrip1OO764RDBLBfrB4cwzNGDj5OA5ioymC9GkizgWJDUg=="],

    "requires-port": ["requires-port@1.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/requires-port/-/requires-port-1.0.0.tgz", {}, "sha512-KigOCHcocU3XODJxsu8i/j8T9tzT4adHiecwORRQ0ZZFcp7ahwXuRU1m+yuO90C5ZUyGeGfocHDI14M3L3yDAQ=="],
    ...
    "safer-buffer": ["safer-buffer@2.1.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/safer-buffer/-/safer-buffer-2.1.2.tgz", {}, "sha512-YZo3K82SD7Riyi0E1EQPojLz7kpepnSQI9IyPbHHg1XXXevb5dJI7tpyN2ADxGcQbHG7vcyRHk0cbwqcQriUtg=="],

    "saxes": ["saxes@6.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/saxes/-/saxes-6.0.0.tgz", { "dependencies": { "xmlchars": "^2.2.0" } }, "sha512-xAg7SOnEhrm5zI3puOOKyy1OMcMlIJZYNJY7xLBwSze0UjhPLnWfj2GF2EpT0jmzaJKIWKHLsaSSajf35bcYnA=="],
    ...
    "siginfo": ["siginfo@2.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/siginfo/-/siginfo-2.0.0.tgz", {}, "sha512-ybx0WO1/8bSBLEWXZvEd7gMW3Sn3JFlW3TvX1nREbDLRNQNaeNN8WK0meBwPdAaOI7TtRRRJn/Es1zhrrCHu7g=="],
    ...
    "source-map": ["source-map@0.6.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/source-map/-/source-map-0.6.1.tgz", {}, "sha512-UjgapumWlbMhkBgzT7Ykc5YXUT46F0iKu8SGXq0bcwP5dz/h0Plj6enJqjz1Zbq2l5WaqYnrVbwWOWMyF3F47g=="],

    "source-map-js": ["source-map-js@1.2.1", "", {}, "sha512-UXWMKhLOwVKb728IUtQPXxfYU+usdybtUrK/8uGE8CQMvrhOpwvzDBwj0QhSL7MQc7vIsISBG8VQ8+IDQxpfQA=="],

    "stackback": ["stackback@0.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/stackback/-/stackback-0.0.2.tgz", {}, "sha512-1XMJE5fQo1jGH6Y/7ebnwPOBEkIEnT4QF32d5R1+VXdXveM0IBMJt8zfaxX1P3QhVwrYe+576+jkANtSS2mBbw=="],

    "std-env": ["std-env@3.10.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/std-env/-/std-env-3.10.0.tgz", {}, "sha512-5GS12FdOZNliM5mAOxFRg7Ir0pWz8MdpYm6AY6VPkGpbA7ZzmbzNcBJQ0GPvvyWgcY7QAhCgf9Uy89I03faLkg=="],
    ...
    "strip-indent": ["strip-indent@3.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/strip-indent/-/strip-indent-3.0.0.tgz", { "dependencies": { "min-indent": "^1.0.0" } }, "sha512-laJTa3Jb+VQpaC6DseHhF7dXVqHTfJPCRDaEbid/drOhgitgYku/letMUqOXFoWV0zIIUbjpdH2t+tYj4bQMRQ=="],

    "strip-json-comments": ["strip-json-comments@3.1.1", "", {}, "sha512-6fPc+R4ihwqP6N/aIv2f1gMH8lOVtWQHoqC4yK6oSDVVocumAsfCqjkXnqiYMhmMwS/mEHLp7Vehlt3ql6lEig=="],

    "strip-literal": ["strip-literal@3.1.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/strip-literal/-/strip-literal-3.1.0.tgz", { "dependencies": { "js-tokens": "^9.0.1" } }, "sha512-8r3mkIM/2+PpjHoOtiAW8Rg3jJLHaV7xPwG+YRGrv6FP0wwk/toTpATxWYOW0BKdWwl82VT2tFYi5DlROa0Mxg=="],
    ...
    "symbol-tree": ["symbol-tree@3.2.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/symbol-tree/-/symbol-tree-3.2.4.tgz", {}, "sha512-9QNk5KwDF+Bvz+PyObkmSYjI5ksVUYtjW7AU22r2NKcfLJcXp96hkDWU3+XndOsUb+AQ9QhfzfCT2O+CNWT5Tw=="],
    ...
    "tinybench": ["tinybench@2.9.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tinybench/-/tinybench-2.9.0.tgz", {}, "sha512-0+DUvqWMValLmha6lr4kD8iAMK1HzV0/aKnCtWb9v9641TnP/MFb7Pc2bxoxQjTXAErryXVgUOfv2YqNllqGeg=="],

    "tinyexec": ["tinyexec@0.3.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tinyexec/-/tinyexec-0.3.2.tgz", {}, "sha512-KQQR9yN7R5+OSwaK0XQoj22pwHoTlgYqmUscPYoknOoWCWfj/5/ABTMRi69FrKU5ffPVh5QcFikpWJI/P1ocHA=="],

    "tinyglobby": ["tinyglobby@0.2.16", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tinyglobby/-/tinyglobby-0.2.16.tgz", { "dependencies": { "fdir": "^6.5.0", "picomatch": "^4.0.4" } }, "sha512-pn99VhoACYR8nFHhxqix+uvsbXineAasWm5ojXoN8xEwK5Kd3/TrhNn1wByuD52UxWRLy8pu+kRMniEi6Eq9Zg=="],

    "tinypool": ["tinypool@1.1.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tinypool/-/tinypool-1.1.1.tgz", {}, "sha512-Zba82s87IFq9A9XmjiX5uZA/ARWDrB03OHlq+Vw1fSdt0I+4/Kutwy8BP4Y/y/aORMo61FQ0vIb5j44vSo5Pkg=="],

    "tinyrainbow": ["tinyrainbow@2.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tinyrainbow/-/tinyrainbow-2.0.0.tgz", {}, "sha512-op4nsTR47R6p0vMUUoYl/a+ljLFVtlfaXkLQmqfLR1qHma1h/ysYk4hEXZ880bf2CYgTskvTa/e196Vd5dDQXw=="],

    "tinyspy": ["tinyspy@4.0.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tinyspy/-/tinyspy-4.0.4.tgz", {}, "sha512-azl+t0z7pw/z958Gy9svOTuzqIk6xq+NSheJzn5MMWtWTFywIacg2wUlzKFGtt3cthx0r2SxMK0yzJOR0IES7Q=="],

    "to-regex-range": ["to-regex-range@5.0.1", "", { "dependencies": { "is-number": "^7.0.0" } }, "sha512-65P7iz6X5yEr1cwcgvQxbbIw7Uk3gOy5dIdtZ4rDveLqhrdJP+Li/Hx6tyK0NEb+2GCyneCMJiGqrADCSNk8sQ=="],

    "tough-cookie": ["tough-cookie@4.1.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tough-cookie/-/tough-cookie-4.1.4.tgz", { "dependencies": { "psl": "^1.1.33", "punycode": "^2.1.1", "universalify": "^0.2.0", "url-parse": "^1.5.3" } }, "sha512-Loo5UUvLD9ScZ6jh8beX1T6sO1w2/MpCRpEP7V280GKMVUQ0Jzar2U3UJPsrdbziLEMMhu3Ujnq//rhiFuIeag=="],

    "tr46": ["tr46@3.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/tr46/-/tr46-3.0.0.tgz", { "dependencies": { "punycode": "^2.1.1" } }, "sha512-l7FvfAHlcmulp8kr+flpQZmVwtu7nfRV7NZujtN0OqES8EL4O4e0qqzL0DC5gAvx/ZC/9lk6rhcUwYvkBnBnYA=="],
    ...
    "universalify": ["universalify@0.2.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/universalify/-/universalify-0.2.0.tgz", {}, "sha512-CJ1QgKmNg3CwvAv/kOFmtnEN05f0D/cn9QntgNOQlQF9dgvVTHj3t+8JPdjqawCHk7V/KA+fbUqzZ9XWhcqPUg=="],
    ...
    "url-parse": ["url-parse@1.5.10", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/url-parse/-/url-parse-1.5.10.tgz", { "dependencies": { "querystringify": "^2.1.1", "requires-port": "^1.0.0" } }, "sha512-WypcfiRhfeUP9vvF0j6rw0J3hrWrw6iZv3+22h6iRMJ/8z1Tj6XfLP4DsUix5MhMPnXpiHDoKyoZ/bdCkwBCiQ=="],
    ...
    "vitest": ["vitest@3.2.4", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/vitest/-/vitest-3.2.4.tgz", { "dependencies": { "@types/chai": "^5.2.2", "@vitest/expect": "3.2.4", "@vitest/mocker": "3.2.4", "@vitest/pretty-format": "^3.2.4", "@vitest/runner": "3.2.4", "@vitest/snapshot": "3.2.4", "@vitest/spy": "3.2.4", "@vitest/utils": "3.2.4", "chai": "^5.2.0", "debug": "^4.4.1", "expect-type": "^1.2.1", "magic-string": "^0.30.17", "pathe": "^2.0.3", "picomatch": "^4.0.2", "std-env": "^3.9.0", "tinybench": "^2.9.0", "tinyexec": "^0.3.2", "tinyglobby": "^0.2.14", "tinypool": "^1.1.1", "tinyrainbow": "^2.0.0", "vite": "^5.0.0 || ^6.0.0 || ^7.0.0-0", "vite-node": "3.2.4", "why-is-node-running": "^2.3.0" }, "peerDependencies": { "@edge-runtime/vm": "*", "@types/debug": "^4.1.12", "@types/node": "^18.0.0 || ^20.0.0 || >=22.0.0", "@vitest/browser": "3.2.4", "@vitest/ui": "3.2.4", "happy-dom": "*", "jsdom": "*" }, "optionalPeers": ["@edge-runtime/vm", "@types/debug", "@types/node", "@vitest/browser", "@vitest/ui", "happy-dom", "jsdom"], "bin": { "vitest": "vitest.mjs" } }, "sha512-LUCP5ev3GURDysTWiP47wRRUpLKMOfPh+yKTx3kVIEiu5KOMeqzpnYNsKyOoVrULivR8tLcks4+lga33Whn90A=="],

    "w3c-xmlserializer": ["w3c-xmlserializer@4.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/w3c-xmlserializer/-/w3c-xmlserializer-4.0.0.tgz", { "dependencies": { "xml-name-validator": "^4.0.0" } }, "sha512-d+BFHzbiCx6zGfz0HyQ6Rg69w9k19nviJspaj4yNscGjrHu94sVP+aRm75yEbCh+r2/yR+7q6hux9LVtbuTGBw=="],

    "webidl-conversions": ["webidl-conversions@7.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/webidl-conversions/-/webidl-conversions-7.0.0.tgz", {}, "sha512-VwddBukDzu71offAQR975unBIGqfKZpM+8ZX6ySk8nYhVoo5CYaZyzt3YBvYtRtO+aoGlqxPg/B87NGVZ/fu6g=="],

    "whatwg-encoding": ["whatwg-encoding@2.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/whatwg-encoding/-/whatwg-encoding-2.0.0.tgz", { "dependencies": { "iconv-lite": "0.6.3" } }, "sha512-p41ogyeMUrw3jWclHWTQg1k05DSVXPLcVxRTYsXUk+ZooOCZLcoYgPZ/HL/D/N+uQPOtcp1me1WhBEaX02mhWg=="],

    "whatwg-mimetype": ["whatwg-mimetype@3.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/whatwg-mimetype/-/whatwg-mimetype-3.0.0.tgz", {}, "sha512-nt+N2dzIutVRxARx1nghPKGv1xHikU7HKdfafKkLNLindmPU/ch3U31NOCGGA/dmPcmb1VlofO0vnKAcsm0o/Q=="],

    "whatwg-url": ["whatwg-url@11.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/whatwg-url/-/whatwg-url-11.0.0.tgz", { "dependencies": { "tr46": "^3.0.0", "webidl-conversions": "^7.0.0" } }, "sha512-RKT8HExMpoYx4igMiVMY83lN6UeITKJlBQ+vR/8ZJ8OCdSiN3RwCq+9gH0+Xzj0+5IrM6i4j/6LuvzbZIQgEcQ=="],

    "which": ["which@2.0.2", "", { "dependencies": { "isexe": "^2.0.0" }, "bin": { "node-which": "bin/node-which" } }, "sha512-BLI3Tl1TW3Pvl70l3yq3Y64i+awpwXqsGBYWkkqMtnbXgrMD+yj7rhW0kuEDxzJaYXGjEW5ogapKNMEKNMjibA=="],

    "why-is-node-running": ["why-is-node-running@2.3.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/why-is-node-running/-/why-is-node-running-2.3.0.tgz", { "dependencies": { "siginfo": "^2.0.0", "stackback": "0.0.2" }, "bin": { "why-is-node-running": "cli.js" } }, "sha512-hUrmaWBdVDcxvYqnyh09zunKzROWjbZTiNy8dBEjkS7ehEDQibXJ7XvlmtbwuTclUiIyN+CyXQD4Vmko8fNm8w=="],
    ...
    "ws": ["ws@8.20.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/ws/-/ws-8.20.0.tgz", { "peerDependencies": { "bufferutil": "^4.0.1", "utf-8-validate": ">=5.0.2" }, "optionalPeers": ["bufferutil", "utf-8-validate"] }, "sha512-sAt8BhgNbzCtgGbt2OxmpuryO63ZoDk/sqaB/znQm94T4fCEsy/yV+7CdC1kJhOU9lboAEU7R3kquuycDoibVA=="],

    "xml-name-validator": ["xml-name-validator@4.0.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/xml-name-validator/-/xml-name-validator-4.0.0.tgz", {}, "sha512-ICP2e+jsHvAj2E2lIHxa5tjXRlKDJo4IdvPvCXbXQGdzSfmSpNVyIKMvoZHjDY9DP0zV17iI85o90vRFXNccRw=="],

    "xmlchars": ["xmlchars@2.2.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/xmlchars/-/xmlchars-2.2.0.tgz", {}, "sha512-JZnDKK8B0RCDw84FNdDAIpZK+JuJw+s7Lz8nksI7SIuU3UXJJslUthsi+uWBUYOwPFwW7W7PRLRfUKpxjtjFCw=="],
    ...
    "@testing-library/dom/aria-query": ["aria-query@5.3.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/aria-query/-/aria-query-5.3.0.tgz", { "dependencies": { "dequal": "^2.0.3" } }, "sha512-b0P0sZPKtyu8HkeRAfCq0IfURZK+SuwMjY1UXGBU27wpAiTwQAIlq56IbIO+ytk/JjS1fMR14ee5WBBfKi5J6A=="],

    "@testing-library/dom/dom-accessibility-api": ["dom-accessibility-api@0.5.16", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/dom-accessibility-api/-/dom-accessibility-api-0.5.16.tgz", {}, "sha512-X7BJ2yElsnOJ30pZF4uIIDfBEVgF4XEBxL9Bxhy6dnrm5hkzqmsWHGTiHqRiITNhMyFLyAiWndIJP7Z1NTteDg=="],

    "@typescript-eslint/eslint-plugin/ignore": ["ignore@7.0.5", "", {}, "sha512-Hs59xBNfUIunMFgWAbGX5cq6893IbWg4KnrjbYwX3tx0ztorVgTDA6B2sxf8ejHJ4wz8BqGUMYlnzNBer5NvGg=="],

    "@typescript-eslint/typescript-estree/minimatch": ["minimatch@9.0.5", "", { "dependencies": { "brace-expansion": "^2.0.1" } }, "sha512-G6T0ZX48xgozx7587koeX9Ys2NYy6Gmv//P89sEte9V9whIapMNF4idKxnW2QtCcLiTWlb/wfCabAtAFWhhBow=="],

    "agent-base/debug": ["debug@4.4.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/debug/-/debug-4.4.3.tgz", { "dependencies": { "ms": "^2.1.3" } }, "sha512-RGwwWnwQvkVfavKVt22FGLw+xYSdzARwm0ru6DhTVA3umU5hZc28V3kO4stgYryrTlLpuvgI9GiijltAjNbcqA=="],

    "anymatch/picomatch": ["picomatch@2.3.1", "", {}, "sha512-JU3teHTNjmE2VCGFzuY8EXzCDVwEqB2a8fsIvwaStHhAWJEeVd1o1QD80CU6+ZdEXXSLbSsuLwJjkCBWqRQUVA=="],

    "chokidar/glob-parent": ["glob-parent@5.1.2", "", { "dependencies": { "is-glob": "^4.0.1" } }, "sha512-AOIgSQCepiJYwP3ARnGx+5VnTu2HBYdzbGP45eLw1vr3zB3vZLeyed1sC9hnbcOc9/SrMyM5RPQrkGz4aS9Zow=="],

    "cssstyle/cssom": ["cssom@0.3.8", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/cssom/-/cssom-0.3.8.tgz", {}, "sha512-b0tGHbfegbhPJpxpiBPU2sCkigAqtM9O121le6bbOlgyV+NyGyCmVfJ6QW9eRjz8CpNfWEOYBIMIGRYkLwsIYg=="],

    "fast-glob/glob-parent": ["glob-parent@5.1.2", "", { "dependencies": { "is-glob": "^4.0.1" } }, "sha512-AOIgSQCepiJYwP3ARnGx+5VnTu2HBYdzbGP45eLw1vr3zB3vZLeyed1sC9hnbcOc9/SrMyM5RPQrkGz4aS9Zow=="],

    "glob/minimatch": ["minimatch@9.0.5", "", { "dependencies": { "brace-expansion": "^2.0.1" } }, "sha512-G6T0ZX48xgozx7587koeX9Ys2NYy6Gmv//P89sEte9V9whIapMNF4idKxnW2QtCcLiTWlb/wfCabAtAFWhhBow=="],

    "http-proxy-agent/debug": ["debug@4.4.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/debug/-/debug-4.4.3.tgz", { "dependencies": { "ms": "^2.1.3" } }, "sha512-RGwwWnwQvkVfavKVt22FGLw+xYSdzARwm0ru6DhTVA3umU5hZc28V3kO4stgYryrTlLpuvgI9GiijltAjNbcqA=="],

    "https-proxy-agent/debug": ["debug@4.4.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/debug/-/debug-4.4.3.tgz", { "dependencies": { "ms": "^2.1.3" } }, "sha512-RGwwWnwQvkVfavKVt22FGLw+xYSdzARwm0ru6DhTVA3umU5hZc28V3kO4stgYryrTlLpuvgI9GiijltAjNbcqA=="],

    "micromatch/picomatch": ["picomatch@2.3.1", "", {}, "sha512-JU3teHTNjmE2VCGFzuY8EXzCDVwEqB2a8fsIvwaStHhAWJEeVd1o1QD80CU6+ZdEXXSLbSsuLwJjkCBWqRQUVA=="],

    "postcss-nested/postcss-selector-parser": ["postcss-selector-parser@6.1.2", "", { "dependencies": { "cssesc": "^3.0.0", "util-deprecate": "^1.0.2" } }, "sha512-Q8qQfPiZ+THO/3ZrOrO0cJJKfpYCagtMUkXbnEfmgUjwXg6z/WBeOyS9APBBPCTSiDV+s4SwQGu8yFsiMRIudg=="],

    "pretty-format/ansi-styles": ["ansi-styles@5.2.0", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/ansi-styles/-/ansi-styles-5.2.0.tgz", {}, "sha512-Cxwpt2SfTzTtXcfOlzGEee8O+c+MmUgGrNiBcXnuWxuFJHe6a5Hz7qwhwe5OgaSYI0IJvkLqWX1ASG+cJOkEiA=="],

    "pretty-format/react-is": ["react-is@17.0.2", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/react-is/-/react-is-17.0.2.tgz", {}, "sha512-w2GsyukL62IJnlaff/nRegPQR94C/XXamvMWmSHRJ4y7Ts/4ocGRmTHvOs8PSE6pB3dWOrD/nueuU5sduBsQ4w=="],
    ...
    "strip-literal/js-tokens": ["js-tokens@9.0.1", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/js-tokens/-/js-tokens-9.0.1.tgz", {}, "sha512-mxa9E9ITFOt0ban3j6L5MpjwegGz6lBQmM1IJkWeBZGcMxto50+eWdjC/52xDbS2vy0k7vIMK0Fe2wfL9OQSpQ=="],
    ...
    "vite-node/debug": ["debug@4.4.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/debug/-/debug-4.4.3.tgz", { "dependencies": { "ms": "^2.1.3" } }, "sha512-RGwwWnwQvkVfavKVt22FGLw+xYSdzARwm0ru6DhTVA3umU5hZc28V3kO4stgYryrTlLpuvgI9GiijltAjNbcqA=="],

    "vitest/debug": ["debug@4.4.3", "https://europe-west4-npm.pkg.dev/lovable-core-prod/sandbox-npm-cache/debug/-/debug-4.4.3.tgz", { "dependencies": { "ms": "^2.1.3" } }, "sha512-RGwwWnwQvkVfavKVt22FGLw+xYSdzARwm0ru6DhTVA3umU5hZc28V3kO4stgYryrTlLpuvgI9GiijltAjNbcqA=="],
...
const PLACEHOLDER_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300" viewBox="0 0 400 300" fill="none">
  <rect width="400" height="300" rx="24" fill="#F4F4F5"/>
  <path d="M120 210L180 150L225 195L290 125L340 210H120Z" fill="#D4D4D8"/>
  <circle cx="160" cy="110" r="24" fill="#A1A1AA"/>
</svg>`;

const FAVICON_ICO = `placeholder favicon`;

const INDEX_CSS_BASE = `@import url("https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&family=Playfair+Display:wght@400;500;600;700;800;900&family=Bebas+Neue&family=Barlow:wght@300;400;500;600;700&display=swap");
...
@layer base {
  :root {
    ...
    --gradient-hero: linear-gradient(135deg, hsl(var(--primary) / 0.14), hsl(var(--background)) 55%, hsl(var(--accent) / 0.22));
    --gradient-surface: linear-gradient(180deg, hsl(var(--background)) 0%, hsl(var(--secondary) / 0.92) 100%);
    --shadow-soft: 0 20px 45px -30px rgb(15 23 42 / 0.28);
    --shadow-card: 0 18px 40px -28px rgb(15 23 42 / 0.22);
    --shadow-card-hover: 0 24px 60px -32px rgb(15 23 42 / 0.32);
  ...
  .dark {
    ...
    --gradient-hero: linear-gradient(140deg, hsl(var(--primary) / 0.12), hsl(var(--background)) 52%, hsl(217.2 91.2% 59.8% / 0.28));
    --gradient-surface: linear-gradient(180deg, hsl(var(--background)) 0%, hsl(var(--card)) 100%);
  }
}
...
@layer base {
  * {
  ...
  body {
    ...
    background-image:
      radial-gradient(circle at top, hsl(var(--primary) / 0.06), transparent 28%),
...
@layer utilities {
  .text-gradient {
    ...
    background-image: linear-gradient(135deg, hsl(var(--foreground)), hsl(var(--primary) / 0.72));
  }

  .surface-card {
    background-image: linear-gradient(180deg, hsl(var(--card) / 0.98), hsl(var(--card) / 0.92));
  ...
  .section-shell::before {
    ...
    inset: 0;
    pointer-events: none;
    background: linear-gradient(180deg, hsl(var(--background) / 0), hsl(var(--primary) / 0.03));
    opacity: 0.8;
  }
...
__DESIGN_FAMILY_THEME__`;

function buildIndexCss(family: DesignFamilyId): string {
  return INDEX_CSS_BASE.replace(
    "__DESIGN_FAMILY_THEME__",
    getDesignFamilyCss(family)
  );
}
...
const ASSETS_GITKEEP = ``;

// Inline SVG data-URI placeholders.  These are embedded directly in the
// generated-images module so the preview iframe can render them without
// any network requests.  "/placeholder.svg" would resolve against the
// parent Next.js origin inside a sandboxed srcdoc iframe, causing
// ERR_TOO_MANY_REDIRECTS.  Data-URIs are self-contained and safe.
const PLACEHOLDER_HERO_SVG = `data:image/svg+xml;base64,${Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1000" fill="none"><defs><linearGradient id="g" x1="0" y1="0" x2="1600" y2="1000" gradientUnits="userSpaceOnUse"><stop stop-color="#1a1a2e"/><stop offset="1" stop-color="#16213e"/></linearGradient></defs><rect width="1600" height="1000" fill="url(#g)"/><rect x="80" y="80" width="1440" height="840" rx="32" fill="#1a1a2e" fill-opacity=".6" stroke="#e2e8f0" stroke-opacity=".08"/><rect x="200" y="340" width="500" height="32" rx="16" fill="#e2e8f0" fill-opacity=".12"/><rect x="200" y="400" width="340" height="20" rx="10" fill="#e2e8f0" fill-opacity=".08"/><circle cx="1100" cy="500" r="180" fill="#e2e8f0" fill-opacity=".06" stroke="#e2e8f0" stroke-opacity=".1"/></svg>`).toString("base64")}`;

const PLACEHOLDER_SUPPORT_SVG = `data:image/svg+xml;base64,${Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1000" fill="none"><defs><linearGradient id="g" x1="0" y1="0" x2="1600" y2="1000" gradientUnits="userSpaceOnUse"><stop stop-color="#1e293b"/><stop offset="1" stop-color="#0f172a"/></linearGradient></defs><rect width="1600" height="1000" fill="url(#g)"/><rect x="100" y="100" width="640" height="800" rx="28" fill="#e2e8f0" fill-opacity=".06"/><rect x="860" y="200" width="600" height="260" rx="24" fill="#e2e8f0" fill-opacity=".08"/><rect x="860" y="520" width="600" height="260" rx="24" fill="#e2e8f0" fill-opacity=".05"/></svg>`).toString("base64")}`;

const PLACEHOLDER_GALLERY_SVG = `data:image/svg+xml;base64,${Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1000" fill="none"><defs><linearGradient id="g" x1="0" y1="0" x2="1600" y2="1000" gradientUnits="userSpaceOnUse"><stop stop-color="#18181b"/><stop offset="1" stop-color="#27272a"/></linearGradient></defs><rect width="1600" height="1000" fill="url(#g)"/><rect x="60" y="60" width="480" height="400" rx="24" fill="#e2e8f0" fill-opacity=".07"/><rect x="560" y="60" width="480" height="400" rx="24" fill="#e2e8f0" fill-opacity=".05"/><rect x="1060" y="60" width="480" height="400" rx="24" fill="#e2e8f0" fill-opacity=".07"/><rect x="60" y="520" width="740" height="420" rx="24" fill="#e2e8f0" fill-opacity=".05"/><rect x="820" y="520" width="720" height="420" rx="24" fill="#e2e8f0" fill-opacity=".07"/></svg>`).toString("base64")}`;

const PLACEHOLDER_DETAIL_SVG = `data:image/svg+xml;base64,${Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1000" fill="none"><defs><linearGradient id="g" x1="0" y1="0" x2="1600" y2="1000" gradientUnits="userSpaceOnUse"><stop stop-color="#1c1917"/><stop offset="1" stop-color="#292524"/></linearGradient></defs><rect width="1600" height="1000" fill="url(#g)"/><rect x="200" y="150" width="1200" height="700" rx="32" fill="#e2e8f0" fill-opacity=".06" stroke="#e2e8f0" stroke-opacity=".08"/><rect x="300" y="600" width="400" height="24" rx="12" fill="#e2e8f0" fill-opacity=".1"/><rect x="300" y="650" width="260" height="16" rx="8" fill="#e2e8f0" fill-opacity=".06"/></svg>`).toString("base64")}`;

const GENERATED_IMAGES_TS = `export const projectImages = {
  hero: "${PLACEHOLDER_HERO_SVG}",
  support: "${PLACEHOLDER_SUPPORT_SVG}",
  gallery1: "${PLACEHOLDER_GALLERY_SVG}",
  gallery2: "${PLACEHOLDER_GALLERY_SVG}",
  gallery3: "${PLACEHOLDER_GALLERY_SVG}",
  detail1: "${PLACEHOLDER_DETAIL_SVG}",
} as const;
...
const HOOKS_USE_MOBILE_TS = `import { useEffect, useState } from "react";
...
const PAGES_HOME_TSX = `import App from "../App";
...
const TEST_APP_TEST_TSX = `import { render, screen } from "@testing-library/react";
import App from "../App";
...
> {
  ...
  componentDidCatch(error: Error) {
    const el = document.getElementById("cenate-error");
    if (el) {
      el.style.display = "block";
      el.textContent = "Render error: " + error.message;
    }
  }
  render() {
    if (this.state.error) {
...
// Fallback App.tsx — only injected if the LLM doesn't generate one.
// NOT in CRITICAL_PATHS so LLM version always takes precedence.
// The signature comment lets pre-flight and messaging detect when this
// placeholder is still active (i.e. real generation failed or was missing).
export const FALLBACK_APP_SIGNATURE = "__CENATE_SCAFFOLD_FALLBACK__";

const FALLBACK_APP_TSX = `// ${FALLBACK_APP_SIGNATURE}
import Section from "@/components/ui/section";
import Container from "@/components/ui/container";
...
function buildScaffoldFiles(family: DesignFamilyId): Record<string, string> {
  return {
  // Config
  ...
  "tailwind.config.ts": TAILWIND_CONFIG_TS,
  ...
  "src/App.tsx": FALLBACK_APP_TSX,
  "src/index.css": buildIndexCss(family),
  "src/App.css": APP_CSS,
  "src/vite-env.d.ts": VITE_ENV_D_TS,
  "src/assets/.gitkeep": ASSETS_GITKEEP,
  "src/assets/generated-images.ts": GENERATED_IMAGES_TS,
  "src/hooks/use-mobile.ts": HOOKS_USE_MOBILE_TS,
  "src/pages/Home.tsx": PAGES_HOME_TSX,
  "src/test/setup.ts": TEST_SETUP_TS,
  "src/test/App.test.tsx": TEST_APP_TEST_TSX,
  // Utility
  "src/lib/utils.ts": UTILS_TS,
  // UI Primitives — Radix-backed
  "src/components/ui/button.tsx": BUTTON_TSX,
  "src/components/ui/accordion.tsx": ACCORDION_TSX,
  ...
  "src/components/ui/section.tsx": SECTION_TSX,
  "src/components/ui/container.tsx": CONTAINER_TSX,
  "src/components/ui/heading.tsx": HEADING_TSX,
  "src/components/ui/separator.tsx": SEPARATOR_TSX,
  "src/components/ui/aspect-ratio.tsx": ASPECT_RATIO_TSX,
  "src/components/ui/testimonial.tsx": TESTIMONIAL_TSX,
  "src/components/ui/stats-band.tsx": STATS_BAND_TSX,
  "src/components/ui/gallery.tsx": GALLERY_TSX,
  "src/components/ui/mobile-nav.tsx": MOBILE_NAV_TSX,
  };
}

export const REQUIRED_SCAFFOLD_FILES = [
  "package.json",
  "bun.lock",
  "index.html",
  "vite.config.ts",
...
export const REQUIRED_SCAFFOLD_DIRECTORIES = [
  "src/components/",
  "src/lib/",
  "src/pages/",
  "src/hooks/",
...
export const PACKAGE_BASELINE_DEPENDENCIES = [
  "react",
  "react-dom",
  "clsx",
...
export const PACKAGE_BASELINE_DEV_DEPENDENCIES = [
  "vite",
  "typescript",
  "tailwindcss",
  "postcss",
...
/**
 * Returns the full scaffold file map. Used by the prompt to list
 * available primitives and by the injection function.
 */
export function getScaffoldFiles(): Record<string, string> {
  return buildScaffoldFiles("modern_minimal");
}

/**
 * Critical scaffold files that MUST use the scaffold version even if
 * the LLM generated its own. This prevents broken entry points,
 * missing utilities, and incompatible UI primitives.
 */
const CRITICAL_PATHS = new Set([
  ...
  "src/components/ui/stats-band.tsx",
  "src/components/ui/gallery.tsx",
  "src/components/ui/mobile-nav.tsx",
]);

/**
 * Remove an existing ===FILE:path=== block from raw output.
 */
function removeFileBlock(raw: string, filePath: string): string {
  ...
  // Remove from start of marker to end of END_FILE marker + trailing newline
...
const SECTION_PRIMITIVE_IMPORTS = [
  { path: "@/components/ui/button", clause: 'import Button from "@/components/ui/button";', symbols: ["Button"] },
  { path: "@/components/ui/input", clause: 'import Input from "@/components/ui/input";', symbols: ["Input"] },
  { path: "@/components/ui/textarea", clause: 'import Textarea from "@/components/ui/textarea";', symbols: ["Textarea"] },
  { path: "@/components/ui/badge", clause: 'import Badge from "@/components/ui/badge";', symbols: ["Badge"] },
  { path: "@/components/ui/card", clause: 'import Card from "@/components/ui/card";', symbols: ["Card"] },
  { path: "@/components/ui/section", clause: 'import Section from "@/components/ui/section";', symbols: ["Section"] },
  { path: "@/components/ui/container", clause: 'import Container from "@/components/ui/container";', symbols: ["Container"] },
  { path: "@/components/ui/heading", clause: 'import Heading from "@/components/ui/heading";', symbols: ["Heading"] },
  { path: "@/components/ui/separator", clause: 'import Separator from "@/components/ui/separator";', symbols: ["Separator"] },
  {
  ...
  },
  {
    ...
    clause: 'import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs";',
  ...
  },
  {
    ...
    clause: 'import { Sheet, SheetTrigger, SheetContent, SheetClose } from "@/components/ui/sheet";',
    symbols: ["Sheet", "SheetTrigger", "SheetContent", "SheetClose"],
  },
  { path: "@/components/ui/mobile-nav", clause: 'import MobileNav from "@/components/ui/mobile-nav";', symbols: ["MobileNav"] },
  { path: "@/components/ui/aspect-ratio", clause: 'import AspectRatio from "@/components/ui/aspect-ratio";', symbols: ["AspectRatio"] },
  { path: "@/components/ui/testimonial", clause: 'import Testimonial from "@/components/ui/testimonial";', symbols: ["Testimonial"] },
  { path: "@/components/ui/stats-band", clause: 'import StatsBand from "@/components/ui/stats-band";', symbols: ["StatsBand"] },
  { path: "@/components/ui/gallery", clause: 'import Gallery from "@/components/ui/gallery";', symbols: ["Gallery"] },
] as const;

function normalizeSectionPrimitiveImports(content: string): string {
  const missingImports = SECTION_PRIMITIVE_IMPORTS.filter(({ path, symbols }) => {
    if (content.includes(`from "${path}"`) || content.includes(`from '${path}'`)) {
      return false;
    }
    return symbols.some((symbol) =>
      new RegExp(`<${symbol}(?=[\\s>])`).test(content)
    );
  }).map(({ clause }) => clause);
  ...
  return [...prefix, ...missingImports, ...suffix].join("\n");
}

function normalizeGeneratedImagesImport(content: string): string {
  const usesProjectImages = /projectImages\./.test(content);
  const hasImport =
    content.includes('from "@/assets/generated-images"') ||
    content.includes("from '@/assets/generated-images'");

  if (!usesProjectImages || hasImport) {
    return content;
  }
...
function normalizeHeroSupport(content: string): string {
  if (!/<Button\b/.test(content)) {
    return content;
  }

  const hasSupportStructure =
    /teams|clients|projects|years|trusted|certified|award|since|logos|uptime|delivery|support|days|status/i.test(
      content
    ) &&
    /(grid-cols-[234]|flex[^\\n]+gap-[34]|StatsBand|Badge)/.test(content);
  ...
  const supportBlock = `
            <div className="mt-8 grid max-w-xl grid-cols-3 gap-3 text-left sm:gap-4">
              {[
                { value: "120+", label: "Projects shipped" },
                { value: "24/7", label: "Response coverage" },
                { value: "4.9/5", label: "Client rating" },
              ].map((item) => (
                <div key={item.label} className="surface-card rounded-2xl border px-4 py-3">
  ...
  return content.replace(/(<\/Section>\s*$)/, `${supportBlock}\n$1`);
}

function normalizeAppFile(content: string): string {
  let normalized = content;

  if (!/<main[\s>]/.test(normalized)) {
    const openMatch = normalized.match(/return\s*\(\s*<div([^>]*)>/);
    if (openMatch) {
      normalized = normalized.replace(
        /return\s*\(\s*<div([^>]*)>/,
        'return (\n    <main role="main"$1>'
      );
      const lastDivIndex = normalized.lastIndexOf("</div>");
      if (lastDivIndex !== -1) {
        normalized =
          normalized.slice(0, lastDivIndex) +
          "</main>" +
          normalized.slice(lastDivIndex + "</div>".length);
      }
    } else if (/return\s*\(\s*<>/.test(normalized)) {
      // Fragment wrapper (<>...</>) — replace with <main role="main">
      normalized = normalized.replace(
        /return\s*\(\s*<>/,
        'return (\n    <main role="main">'
      );
      const lastFragIndex = normalized.lastIndexOf("</>");
      if (lastFragIndex !== -1) {
        normalized =
          normalized.slice(0, lastFragIndex) +
          "</main>" +
          normalized.slice(lastFragIndex + "</>".length);
      }
    } else if (/return\s*\(\s*<React\.Fragment>/.test(normalized)) {
      // React.Fragment wrapper — replace with <main role="main">
      normalized = normalized.replace(
        /return\s*\(\s*<React\.Fragment>/,
        'return (\n    <main role="main">'
      );
      const lastFragIndex = normalized.lastIndexOf("</React.Fragment>");
      if (lastFragIndex !== -1) {
        normalized =
          normalized.slice(0, lastFragIndex) +
          "</main>" +
          normalized.slice(lastFragIndex + "</React.Fragment>".length);
      }
    }
  }

  if (/<main[\s>]/.test(normalized) && !/role="main"/.test(normalized)) {
    normalized = normalized.replace(/<main\b/, '<main role="main"');
  }
      ...
      if (/className=/.test(attrs)) {
        return `<main${attrs.replace(
          ...
          (_m, quote: string, value: string) =>
            `className=${quote}${value.includes("min-h-screen") ? value : `min-h-screen bg-background text-foreground ${value}`}${quote}`
        )}>`;
      }
      return `<main className="min-h-screen bg-background text-foreground"${attrs}>`;
    }
  );

  const renderedComponents = Array.from(
    normalized.matchAll(/<([A-Z][A-Za-z0-9]*)\b[^>]*\/?>/g)
  ).map((match) => match[1]);
  const seen = new Set<string>();
  const uniqueRendered = renderedComponents.filter((name) => {
    if (seen.has(name)) return false;
    seen.add(name);
    return true;
  });
  const bodyComponents = uniqueRendered.filter(
    (name) => !["Navbar", "Hero", "Footer"].includes(name)
  );

  if (bodyComponents.length > 5) {
    const removedComponents = bodyComponents.slice(5);
    for (const componentName of removedComponents) {
      normalized = normalized
        .replace(
          new RegExp(`\\n?\\s*<${componentName}\\b[^>]*\\/?>\\s*`, "g"),
          "\n"
        )
        .replace(
          new RegExp(
            `\\n?\\s*<${componentName}\\b[^>]*>[\\s\\S]*?<\\/${componentName}>\\s*`,
            "g"
          ),
          "\n"
        );
    ...
    for (const componentName of removedComponents) {
      normalized = normalized.replace(
        new RegExp(
          `^import\\s+${componentName}\\s+from\\s+["']\\.\\/components\\/${componentName}["'];?\\n?`,
          "m"
        ),
        ""
      );
...
function normalizeGeneratedFile(path: string, content: string): string {
  const safeContent = content
    .replace(/\btext-9xl\b/g, "text-7xl")
    .replace(/\btext-8xl\b/g, "text-7xl");

  if (path === "src/App.tsx") {
    return normalizeAppFile(safeContent);
  }
  if (
    ...
    const withImports = normalizeGeneratedImagesImport(
      normalizeSectionPrimitiveImports(safeContent)
    );
    if (/\/Hero\.tsx$/.test(path)) {
      return normalizeHeroSupport(withImports);
    }
    return withImports;
  }
  return safeContent;
...
function extractDesignFamily(raw: string): DesignFamilyId {
  ...
  try {
    const manifest = JSON.parse(manifestMatch[1].trim()) as {
      designFamily?: string;
    };
    if (
      manifest.designFamily === "editorial_luxury" ||
      manifest.designFamily === "modern_minimal" ||
      manifest.designFamily === "bold_commercial" ||
      manifest.designFamily === "warm_artisan"
    ) {
      return manifest.designFamily;
    }
  } catch {
    return "modern_minimal";
  }
...
/**
 * Inject scaffold files into raw delimited LLM output.
 *
 * Critical files (entry, utils, UI primitives) ALWAYS use the scaffold
 * version — if the LLM generated its own, it is replaced. This prevents
 * broken imports and incompatible components.
 *
 * Config files are injected only if the LLM did not generate them.
 */
export function injectScaffoldFiles(raw: string): string {
  const family = extractDesignFamily(raw);
  const scaffoldFiles = buildScaffoldFiles(family);
  let result = raw.replace(
    /===FILE:(.*?)===\n([\s\S]*?)===END_FILE===/g,
    (_match, filePath: string, content: string) => {
      const normalizedPath = filePath.trim();
      const normalizedContent = normalizeGeneratedFile(
        normalizedPath,
        content.trimEnd()
      );
  ...
  for (const [path, content] of Object.entries(scaffoldFiles)) {
    const existsInOutput = result.includes(`===FILE:${path}===`);

    if (CRITICAL_PATHS.has(path)) {
      // Critical: always use scaffold version — remove LLM's version if present
      if (existsInOutput) {
        result = removeFileBlock(result, path);
      }
      blocks.push(`===FILE:${path}===\n${content}\n===END_FILE===`);
    } else {
      // Non-critical: inject only if missing
      if (!existsInOutput) {
        if (path === "src/App.tsx") {
          console.warn("[scaffold] injecting FALLBACK App.tsx — LLM did not generate src/App.tsx");
        }
        blocks.push(`===FILE:${path}===\n${content}\n===END_FILE===`);
      } else if (path === "src/App.tsx") {
        // Check if the existing App.tsx is our fallback (from a prior injection pass) or real LLM content
        const isFallbackStillActive = result.includes("__CENATE_SCAFFOLD_FALLBACK__");
        if (isFallbackStillActive) {
          console.warn("[scaffold] src/App.tsx exists but is still the FALLBACK — LLM did not replace it");
        } else {
          console.log("[scaffold] real src/App.tsx found — keeping generated version");
        }
      }
    }
  }
  ...
  // Insert after the manifest so scaffold files appear before section components
  const manifestEndMarker = "===END_MANIFEST===";
  const manifestEnd = result.indexOf(manifestEndMarker);
  if (manifestEnd !== -1) {
    const insertPoint = manifestEnd + manifestEndMarker.length;
    return (
      result.slice(0, insertPoint) + "\n" + insertText + result.slice(insertPoint)
    );
  }
...
/**
 * Prompt text describing available UI primitives. Injected into the
 * system prompt so the LLM knows what's available and how to use it.
 */
export const UI_PRIMITIVES_PROMPT = `=== PRE-INJECTED UI STACK ===

The following files are automatically included in every generated project.
Do NOT generate them — they already exist. Do NOT rebuild buttons, inputs,
cards, accordions, tabs, dialogs, sheets, or section containers from scratch.

DEPENDENCIES (included in package.json, resolved as builtins at runtime):
  @radix-ui/react-slot — polymorphic Slot for Button's asChild prop
  @radix-ui/react-accordion — collapsible FAQ / disclosure sections
  @radix-ui/react-tabs — tabbed content switcher
  @radix-ui/react-dialog — modal dialogs and sheet drawers (mobile nav, modals)
  ...
  clsx — class name utility used by cn()
  tailwindcss-animate — animation utilities for Radix component states

LOVABLE-STYLE SCAFFOLD (present in every generated project):
  bun.lock
  public/favicon.ico
  public/placeholder.svg
  public/robots.txt
...
Entry: src/main.tsx (imports App + index.css + App.css, mounts React)
Utility: src/lib/utils.ts — exports cn(...inputs) using clsx for class concatenation

UI PRIMITIVES (import from @/components/ui/):

  Button [Radix Slot + cva] — import Button from "@/components/ui/button"
    Props: variant ("default"|"destructive"|"outline"|"secondary"|"ghost"|"link"), size ("default"|"sm"|"lg"|"icon"), asChild (boolean), className, children, ref
    Variant usage:
      <Button>Primary CTA</Button>                          — filled primary color
      <Button variant="outline">Secondary</Button>          — bordered, transparent bg
      <Button variant="secondary">Alt action</Button>       — muted fill
      <Button variant="ghost">Subtle</Button>               — no bg, hover highlight
      <Button variant="link">Learn more</Button>            — text link style
      <Button variant="destructive">Delete</Button>         — danger action
    Override with className for niche-specific accent colors: <Button className="bg-amber-600 text-white hover:bg-amber-700">
    Polymorphic: <Button asChild><a href="#section">Link as button</a></Button>

  Badge [cva] — import Badge from "@/components/ui/badge"
    Props: size ("sm"|"md"|"lg"), className, children

  Input [custom] — import Input from "@/components/ui/input"
    Props: label, error, className, plus native input attrs

  Textarea [custom] — import Textarea from "@/components/ui/textarea"
    Props: label, className, plus native textarea attrs

  Card [custom] — import Card from "@/components/ui/card"
    Props: className, children. Rounded container with padding.

  Section [custom] — import Section from "@/components/ui/section"
    Props: id, className, children. Full-width section with vertical padding.

  Container [custom] — import Container from "@/components/ui/container"
    Props: size ("sm"|"md"|"lg"|"xl"|"full"), className. Max-width centered wrapper.

  Heading [custom] — import Heading from "@/components/ui/heading"
    Props: as ("h1"|"h2"|"h3"|"h4"), size ("display"|"h1"|"h2"|"h3"), eyebrow (optional), className

  Separator [custom] — import Separator from "@/components/ui/separator"
    Props: orientation ("horizontal"|"vertical"), className. Visual divider.

  Accordion [Radix] — import { Accordion, AccordionItem, AccordionTrigger, AccordionContent } from "@/components/ui/accordion"
    Use for FAQ sections, collapsible content, disclosure panels.
    Usage: <Accordion type="single" defaultValue="item-1" className="w-full">
      <AccordionItem value="item-1"><AccordionTrigger>Question?</AccordionTrigger><AccordionContent>Answer.</AccordionContent></AccordionItem>
    </Accordion>

  Tabs [Radix] — import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/components/ui/tabs"
    Use for pricing toggles, feature category tabs, content switchers.
    Usage: <Tabs defaultValue="monthly" className="w-full">
      <TabsList><TabsTrigger value="monthly">Monthly</TabsTrigger><TabsTrigger value="annual">Annual</TabsTrigger></TabsList>
      <TabsContent value="monthly">Monthly pricing...</TabsContent>
      <TabsContent value="annual">Annual pricing...</TabsContent>
    </Tabs>

  Dialog [Radix] — import { Dialog, DialogTrigger, DialogContent, DialogClose, DialogTitle, DialogDescription } from "@/components/ui/dialog"
    Use for modal overlays, confirmation dialogs.
    Usage: <Dialog><DialogTrigger asChild><Button>Open</Button></DialogTrigger>
      <DialogContent><DialogTitle>Title</DialogTitle><DialogDescription>Body</DialogDescription></DialogContent>
    </Dialog>

  Sheet [Radix Dialog] — import { Sheet, SheetTrigger, SheetContent, SheetClose } from "@/components/ui/sheet"
    Use for mobile navigation drawers, slide-out panels.
    Props on SheetContent: side ("left"|"right"|"top"|"bottom"), className
    Usage: <Sheet><SheetTrigger asChild><button>Menu</button></SheetTrigger>
      <SheetContent side="right"><nav>...</nav></SheetContent>
    </Sheet>

  MobileNav [Sheet-backed] — import MobileNav from "@/components/ui/mobile-nav"
    Pre-built responsive mobile navigation drawer using Sheet.
    Props: links (array of { label, href }), brandName, ctaLabel?, ctaHref?, className
    Usage: <MobileNav links={[{label:"About",href:"#about"}]} brandName="Brand" ctaLabel="Contact" ctaHref="#contact" />

  AspectRatio [custom] — import AspectRatio from "@/components/ui/aspect-ratio"
    Props: ratio (number, default 16/9), className, children
    Maintains consistent image aspect ratios in galleries and media sections.

  Testimonial [custom] — import Testimonial from "@/components/ui/testimonial"
    Props: quote, author, role?, avatar?, className
    Structured quote presentation with attribution.

  StatsBand [custom] — import StatsBand from "@/components/ui/stats-band"
    Props: stats (array of { value, label }), className
    Full-width metrics/credential band. Pre-composed with Section + Container.

  Gallery [custom] — import Gallery from "@/components/ui/gallery"
    Props: images (array of { src, alt, span? }), className
    Asymmetric image gallery. span: "normal"|"wide"|"tall" controls grid behavior.

STACK COMPOSITION:
  Radix-backed (real behavior primitives): Button (Slot), Accordion, Tabs, Dialog, Sheet
  cva-backed (variant class generation): Button, Badge
  Custom composite (Section-level helpers): StatsBand, Gallery, MobileNav, Testimonial
  Custom Tailwind (structural wrappers): Card, Section, Container, Heading, Input, Textarea, Separator, AspectRatio
  cn(): real clsx utility

USAGE RULES:
- Import and USE these primitives in every section component
- Build sections by composing: Section > Container > content using Heading, Button, Input, Card, Badge, Accordion, Tabs
- Use Accordion for FAQ sections — do NOT build custom disclosure/toggle patterns
- Use Tabs for pricing toggles — do NOT build custom tab state management
- Use Sheet or MobileNav for mobile navigation — do NOT build custom mobile menu state
- Use Dialog for modal overlays — do NOT build custom modal state management
- Use Gallery for image grids — do NOT build custom gallery components inline
- Use StatsBand for metrics/credential bands — do NOT build stats sections from scratch
- Use Testimonial for quote attribution — do NOT build custom quote components inline
- Style via className using Tailwind utilities — primitives provide structure, you provide colors
- cn() from @/lib/utils concatenates classes: cn("base", condition && "conditional", className)

=== END PRE-INJECTED UI STACK ===`;

/**
 * Section composition patterns showing how to use primitives.
 * Included in the prompt as concrete code reference.
 */
export const SECTION_PATTERNS_PROMPT = `=== SECTION COMPOSITION PATTERNS ===

Use these as structural starting points when building sections. All use the pre-injected UI primitives and semantic design tokens.

DESIGN TOKEN SYSTEM:
  The scaffold includes a full semantic CSS variable system. Use these Tailwind classes:
  ...
  - Fonts: font-heading (headings), font-body (body text)
  - Shadows: shadow-sm, shadow-md (or use var(--shadow-card) / var(--shadow-card-hover))
  You can ALSO use raw Tailwind colors (bg-zinc-950, text-amber-400) for niche-specific accents that go beyond the token system.

  CSS custom properties are set in index.css and can be overridden per-niche:
  - --font-heading, --font-body: font families
  - --gradient-hero: hero overlay gradient
  - --shadow-card, --shadow-card-hover: card elevation
  - --primary, --secondary, --accent, etc.: HSL color values

SPLIT HERO:
  <Section className="min-h-[85vh] flex items-center bg-zinc-950 text-white">
    <Container>
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        <div className="max-w-xl">
          <p className="text-xs uppercase tracking-[0.2em] text-primary font-semibold mb-4">Tagline</p>
          <Heading as="h1" size="display" className="font-heading">Headline</Heading>
          <p className="mt-6 text-lg text-muted-foreground max-w-lg">Subtitle</p>
          <div className="mt-8 flex flex-wrap gap-4">
            <Button size="lg">Primary CTA</Button>
            <Button size="lg" variant="outline" className="border-zinc-600 text-zinc-300 hover:bg-zinc-800">Secondary</Button>
          </div>
        </div>
        <div>
          <img src="..." alt="..." className="rounded-2xl object-cover w-full aspect-[4/3]" />
        </div>
      </div>
    </Container>
  </Section>
...
STATS / METRICS BAND:
  <Section className="bg-secondary py-12">
    <Container>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
        <div className="animate-count-up" style={{ animationDelay: "0s", opacity: 0 }}>
          <p className="text-3xl md:text-5xl font-heading font-bold text-primary">27+</p>
    ...
    <Container>
      <div className="grid lg:grid-cols-2 gap-16 items-start">
        <div>
          ...
          <p className="mt-4 text-muted-foreground max-w-md">Supporting text</p>
        </div>
...
CONTENT SPLIT (alternating image + text):
  <Section className="bg-background text-foreground">
    <Container>
      <div className="grid lg:grid-cols-2 gap-16 items-center">
        <img src="..." alt="..." className="rounded-2xl object-cover w-full aspect-[4/3]" />
        <div>
          <Heading size="h2" eyebrow="About Us">Our Story</Heading>
          <p className="mt-6 text-muted-foreground leading-relaxed max-w-lg">Body text...</p>
...
NICHE-SPECIFIC ACCENT OVERRIDE:
  For construction: override --primary to construction orange, --font-heading to Bebas Neue
  For restaurant: override --primary to gold, --font-heading to Playfair Display
  For SaaS: override --primary to brand blue, keep Inter for both
  For agency/portfolio: override --primary to editorial black, --font-heading to serif

  These are set per-project in the generated index.css :root block. Section components use the semantic tokens and get the right colors automatically.

Adapt these patterns to the niche. Change colors, content, and layout proportions. Do not copy them verbatim.
